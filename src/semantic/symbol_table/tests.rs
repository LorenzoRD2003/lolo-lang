...
use crate::{
  ast::expr::VarId,
  semantic::{scope::ScopeArena, symbol::Mutability, symbol_table::SymbolTable, types::Type},
};
...
fn all_symbols_in_scope_returns_correct_symbols() {
  ...
  let c = table.add_symbol(&VarId("c".into()), Type::Int32, Mutability::Mutable, 4..5);
...
fn variable_was_declared_in_current_scope() {
  ...
  table.enter_scope();
  let name = VarId("x".into());
  let symbol = table.add_symbol(&name, Type::Int32, Mutability::Mutable, 0..1);
